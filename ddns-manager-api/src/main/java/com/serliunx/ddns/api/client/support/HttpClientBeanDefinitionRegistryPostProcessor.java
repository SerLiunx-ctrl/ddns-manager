package com.serliunx.ddns.api.client.support;

import com.serliunx.ddns.api.client.annotation.HttpClient;
import com.serliunx.ddns.api.client.decoder.JacksonDecoder;
import com.serliunx.ddns.api.client.encoder.JacksonEncoder;
import feign.Feign;
...
public final class HttpClientBeanDefinitionRegistryPostProcessor implements BeanDefinitionRegistryPostProcessor,
    ...
    private final Class<? extends Annotation> annotatedType = HttpClient.class;
    private final InterfaceWithAnnotationTypeFilter interfaceWithAnnotationTypeFilter =
            new InterfaceWithAnnotationTypeFilter(HttpClient.class);
    ...
    private BeanDefinitionHolder processBeanDefinition(BeanDefinition beanDefinition){
        try {
            ...
            if(httpClient == null){
                throw new RuntimeException("该接口未包含指定的注解: " + httpClient.getClass().getName());
            }
            //获取注解的url
            ...
            //注册bean、生成bean名称
            String name = httpClient.value().isEmpty() ? generateBeanName(beanDefinition) : httpClient.value();
